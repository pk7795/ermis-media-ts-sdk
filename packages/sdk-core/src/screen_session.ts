...
import { JoinInfo, SessionEvent, SessionStatus } from "./session";
export interface ScreenSessionConfig {
    join?: JoinInfo;
}
export class ScreenSession extends EventEmitter {
    ...
    constructor(
        ...
        private cfg: ScreenSessionConfig,
    ) {
        super();
        this.created_at = new Date().getTime();
        console.info("Create screen sharing session", this.created_at);
        ...
        this.peer.onnegotiationneeded = (event) => {
            console.log("[Screen Session] RTCPeer negotiation needed", event);
        ...
        this.peer.onconnectionstatechange = (event) => {
            console.log(
                "[Screen Session] RTCPeer connection state changed",
        ...
        this.peer.oniceconnectionstatechange = (_event) => {
            console.log(
                "[Screen Session] RTCPeer ice state changed",
        ...
        this.peer.ontrack = (event) => {
            ...
            console.warn("[ScreenSession] Received unexpected track in screen sharing session", event.track);
        };
    ...
    receiver(kind: Kind): TrackReceiver {
        console.warn("[ScreenSession] This screen sharing session is designed for sending only. Receiver creation is discouraged.");
    ...
    async connect(token: string, version?: string) {
        this.token = token;
        if (!this.prepareState) {
    ...
    async restartIce() {
        ...
        if (this.conn_id !== res.connId) {
            console.log(
                "Screen Session connect to new server, reset receivers for handling new recv tracks",
            );
            this.conn_id = res.connId;
